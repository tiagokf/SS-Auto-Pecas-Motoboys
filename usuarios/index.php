...
$usuarios = fetchAll("SELECT * FROM usuarios ORDER BY nome");
                    ...
                    <p>
                        ...
                        switch ($error) {
                            ...
                            case 'self_delete':
                                ...
                                break;
                            case 'system_user':
                                echo 'O usuário Sistema não pode ser excluído!';
                                break;
                            case 'in_use':
                                echo 'Este usuário não pode ser excluído pois está vinculado a movimentos!';
                                break;
                            case 'not_found':
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-4 px-6 text-right">
                                <button
                                    ...
                                    class="text-blue-600 hover:text-blue-900 mr-3" title="Editar Usuário">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick="confirmarExclusao(<?php echo $usuario['id']; ?>)"
...
<div id="modal-adicionar"
    ...
    <div class="modal-content bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        <div class="flex justify-between items-center mb-4">
            <h4 class="text-lg font-medium">Adicionar Usuário</h4>
            <button onclick="hideModal('modal-adicionar')" class="text-gray-500 hover:text-gray-700">
                <i class="fas fa-times"></i>
            </button>
        </div>

        <form id="form-adicionar" method="POST" action="../api/usuarios.php">
...
<div id="modal-editar" class="modal fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center hidden">
    <div class="modal-content bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        <div class="flex justify-between items-center mb-4">
            <h4 class="text-lg font-medium">Editar Usuário</h4>
            <button onclick="hideModal('modal-editar')" class="text-gray-500 hover:text-gray-700">
                <i class="fas fa-times"></i>
            </button>
        </div>

        <form id="form-editar" method="POST" action="../api/usuarios.php">
            ...
            <div class="mb-4">
                <label for="edit-senha" class="block text-gray-700 font-medium mb-2">Senha</label>
                <input type="text" id="edit-senha" name="senha"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    required>
            </div>
...
<div id="modal-excluir" class="modal fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center hidden">
    <div class="modal-content bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        ...
        <p class="mb-4">Tem certeza que deseja excluir este usuário? <strong>Todos os registros relacionados a este usuário serão transferidos para o usuário "Sistema"</strong>. Esta ação não pode ser desfeita.</p>

        <form id="form-excluir" method="POST" action="../api/usuarios.php">
...
<script src="<?php echo SITE_URL; ?>/assets/js/usuarios.js"></script>
