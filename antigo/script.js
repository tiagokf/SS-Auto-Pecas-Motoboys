...
function openEditMovimentoModal(button) {
  const movimento = JSON.parse(button.getAttribute('data-movimento'));

  // Preencha os campos do modal com os dados do movimento
  jQuery('#edit_movimento_id').val(movimento.id);
  jQuery('#edit_motoboy_id').val(movimento.motoboy_id);
  jQuery('#edit_usuario_saida_id').val(movimento.usuario_saida_id);
  jQuery('#edit_usuario_retorno_id').val(movimento.usuario_retorno_id);
  jQuery('#edit_data_hora_saida').val(movimento.data_hora_saida ? movimento.data_hora_saida.slice(0, 16) : '');
  jQuery('#edit_data_hora_retorno').val(movimento.data_hora_retorno ? movimento.data_hora_retorno.slice(0, 16) : '');
  jQuery('#edit_descricao').val(movimento.descricao);
}
...
jQuery('#editMovimentoModal').on('hidden.bs.modal', function () {
  jQuery('#edit_movimento_id').empty();
  jQuery('#edit_motoboy_id').empty();
  jQuery('#edit_usuario_saida_id').empty();
  jQuery('#edit_usuario_retorno_id').empty();
  jQuery('#edit_data_hora_saida').val('');
  jQuery('#edit_data_hora_retorno').val('');
  jQuery('#edit_descricao').val('');
});
